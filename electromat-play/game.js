...
(() => {
  ...
  function padSlab(x, y, w, d, live) {
    const p = { fl: [x, y], fr: [x + w, y], br: [x + w + d * 0.56, y - d * 0.4], bl: [x + d * 0.56, y - d * 0.4] };
    const fill = live ? "#cfc8ba" : "#2a2a32";
    const edge = live ? "#00D4F5" : "rgba(0,212,245,0.4)";
    return `<path d="M${[p.fl, p.fr, p.br, p.bl].map((pt) => pt.join(" ")).join(" L")} Z" fill="${fill}" stroke="${edge}" stroke-width="1.1" stroke-linejoin="round"/>`;
  }
  boot();
})();
